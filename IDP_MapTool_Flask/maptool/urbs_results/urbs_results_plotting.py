...
def cap_pro_generate_plot(hdf_path, site_name):
    ...
    fig.write_html(filename + ".html", full_html=False, include_plotlyjs=False, div_id=site_name)
    return(filename)

def cap_sto_c_generate_plot(hdf_path, site_name):
...
def cap_sto_p_generate_plot(hdf_path, site_name):
...
def cap_tra_generate_plot(hdf_path, site_name):
...
def e_pro_in_generate_plot(hdf_path, site_name):
    ...
    filename = site_name + "_e_pro_in.html"
    fig.write_html(filename + ".html", full_html=False, include_plotlyjs=False, div_id=site_name)
    return(site_name + "_e_pro_in")

def e_pro_out_generate_plot(hdf_path, site_name):
    hdf = pd.read_hdf(hdf_path, key='result/e_pro_out')
    ...
    filename = site_name + "_e_pro_out.html"
    fig.write_html(filename + ".html",full_html=False, include_plotlyjs=False, div_id=site_name)
    return(filename)